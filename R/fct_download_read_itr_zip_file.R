...
#'
#' @return A dataframe with itr dat
#' @export
#'
#' @examples
#'
#' \dontrun{ # keep cran check fast
#' df_out <- download_read_itr_zip_file(url_in = 'http://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/ITR/DADOS/itr_cia_aberta_2012.zip', 
...
download_read_itr_zip_file <- function(url_in, 
  ...
  type_files_doc <-as.character(purrr::map(str_split(basename(unzipped_files), '_'), 4))
  type_files_doc <- stringr::str_to_upper(type_files_doc)
  
  type_files_format <- as.character(purrr::map(str_split(basename(unzipped_files), '_'), 5))
